...
function Header({ activeTab, setActiveTab, tabs }:Props) {
    ...
    <div ref={ref}>
      <header css={styleWrapper(headerIsTransparent)}>
        <div tw="flex items-center">
          ...
          <Nav isOpen={isOpen}>
            { tabs.map((tab) => (
              <Tab href={`#${tab.toLocaleLowerCase()}`} key={tab} isActive={activeTab === tab} onClick={() => handleactiveTab(tab)}>
                {tab}
              </Tab>
            ))}
            <button type="button" tw="absolute top-3 right-8 text-3xl cursor-pointer md:hidden" onClick={() => setIsOpen(false)}>
              <Icon name="Close" />
            </button>
          </Nav>
        </div>
        <div tw="flex items-center justify-between space-x-2">
          ...
          <button type="button" tw="cursor-pointer text-3xl hover:text-secondary flex justify-center items-center md:hidden" onClick={() => setIsOpen(true)}>
            <Icon name="Hamburger" />
          </button>
        </div>
      </header>
