...
const stylesLink = (withBorder?: boolean) => [
  ...
  withBorder &&
    tw`text-third border-2 border-third hover:border-secondary w-[50px] h-[50px] rounded-full`,
];
function LinkIcon({ link, Icon, withBorder }: LinkIconProps) {
  return (
    <a href={link} css={stylesLink(withBorder)}>
      {Icon}
    </a>
  );
}
LinkIcon.defaultProps = { withBorder: false };
